...
**Status as of 2026-06-23 (develop @ a0d59b362).**
**Companion to:** `planning/phase_c/REPL_SCHISM_RESOLUTION.md`, `planning/phase_c/REPL_SCHISM_EXECUTION_PLAN.md`.
**Purpose:** Make the schism-resolution Section-1 feature table verifiable. Each row is the answer to "is the boxen REPL ready to be the default?" — and for the C.0.x milestone series the answer is now yes for every row.

The boxen REPL is launched with `frontier-cli --debug-tui`. The linenoise REPL is launched with `frontier-cli` (no flag) and remains the default through milestone C.6.

## Feature comparison

| # | Feature | Linenoise (`repl.c`) | Boxen (`boxen_repl.c`) | Status | Notes |
|---|---------|----------------------|------------------------|--------|-------|
| 1 | Activation | Default `frontier-cli` (no flag) | `--debug-tui` opt-in | DIFFERENT-BUT-INTENTIONAL | Will reverse at C.6. `main.c:852-858` wires `g_cli_options.tui_mode` to `boxen_repl_main()`. |
...
| 14 | Remaining dialog verbs (`dialog.ask` yes/no, `dialog.alert`, `dialog.notify`, `dialog.twoway`, `dialog.threeway`) | Raw terminal IO via `dialog_prompts.c` | Currently fall through to the linenoise path (also under boxen). Phase 2 of the UI bridge will bridge these to boxen modals. | GAP — Phase 2 of UI bridge | Not blocking C.6 (these verbs are rarely reached from dispatched menu scripts today). Tracked as Phase 2. |
| 15 | File dialog verbs (`file.getFileDialog`, etc.) | Behind an `isInteractiveMode()` guard in `file_browser.c`; runs a full-screen TUI when triggered | Currently unreachable from the boxen REPL (same guard). | GAP — Phase 2 of UI bridge | Tracked as Phase 2. No regression vs today — both REPLs currently route to the same code path that's gated off. |
