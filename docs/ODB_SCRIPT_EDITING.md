...
- **[#675](https://github.com/jsavin/Frontier/issues/675) — Sync verification (P1, partially landed).** `tools/verify_virgin_root_sync.sh` walks staged `.ut` files (incremental, default) or the full corpus (`--full`), reads each corresponding script from `Virgin.root` via protocol, normalizes both sides (MacRoman → UTF-8, CR → LF, comment-marker substitution, outline structure-marker stripping), and asserts byte-equality. The pre-commit hook runs the incremental check on every commit that touches `.ut` files (currently warn-only — the corpus carries ~12% historical drift from the original Windows-1252-decoded-as-MacRoman export, predating the verifier; promoting to a hard gate requires a corpus cleanup pass). Three invocation paths: `./tools/verify_virgin_root_sync.sh [--full|--paths FILE...]` direct, `make verify-odb-sync` from repo root, or `cd tests && make verify-odb-sync`. Use `--rewrite` to overwrite drifted `.ut` files with kernel-canonical form (inspect the diff before committing).
...
- **Runtime bidirectional sync (`--ut-sync-dir`).** See `docs/usertalk/UT_SYNC_WORKFLOW.md` for the authoritative agent-facing workflow guide (protocol-first edit/debug, outbound `.ut` mirror, mtime last-write-wins, sharp edges, and the known-limitation issues #697/#700). A live ODB<->.ut mirror exists as a runtime feature, gated by the `--ut-sync-dir DIR` flag (or `FRONTIER_UT_SYNC_DIR` env var; CLI wins). With it active, saved scripts export to `<DIR>/<rootBasename>/<dotted/path>.ut` on exit, and out-of-band `.ut` edits import back on next hydrate (last-write-wins by mtime; ODB authoritative, `.ut` read-only mirror). The authoritative canonicalizer is the C pair `ut_canonicalize_outline_text` / `ut_decanonicalize_outline_text` in `frontier-cli/ut_sync.c` — the Python `normalize_kernel_body` in the git-time verifier is now a parity-tracked reference only (`tests/ut_sync_canonicalize_tests.c` enforces byte-equality). This runtime feature is distinct from the git-time drift verifier above: the verifier checks the two *committed* artifacts match; the sync feature mirrors them *at runtime*. Lifecycle behavior is covered by `tests/ut_sync_lifecycle_test.sh`.
