...
- Vendored libyaml 0.2.5 under `third_party/libyaml/` and updated `tools/strings_compiler/` to load YAML via libyaml; bison/flex work deferred to Phase 2. Header cleanup staged in [`header_cleanup_plan.md`](header_cleanup_plan.md): QuickDraw/UI extraction → TextEncoding cleanup → AppleEvent split → include audit/regression rebuild.
