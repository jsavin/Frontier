...
**Snapshot:** Archived on January 28, 2026

## 2026-01-28 Archive (TCP Verbs 100% Coverage)

### TCP Verbs 100% Coverage - MERGED ✅ (2026-01-28)
- **Impact**: Complete TCP networking implementation with full UserTalk verb coverage
- **PR #361**: Implements 10 remaining verbs, migrates from legacy API, removes 2,406 lines
- **Phase 2 Verbs Added**:
  - `tcp.statusStream()` - Connection status with pending byte count
  - `tcp.getPeerAddress()`, `tcp.getPeerPort()` - Peer information
  - `tcp.myAddress()` - Local address lookup
  - `tcp.readStreamUntil()` - Pattern-based buffered read with timeout
  - `tcp.readStreamBytes()` - Exact byte count read with timeout
  - `tcp.readStreamUntilClosed()` - Read until EOF with timeout
  - `tcp.writeStringToStream()` - Chunked writes with timeout
  - `tcp.writeFileToStream()` - File streaming
  - `tcp.getStats()` - Listener statistics
- **Security Hardening**:
  - Fixed ARM64 `ioctl(FIONREAD)` type mismatch (int vs long)
  - Added slow-trickle DoS protection (35-second throughput window)
  - Optimized pattern matching O(n²) → O(n)
- **Legacy Removal**:
  - Deleted `MacSocketNetEvents.c` (2,263 lines)
  - Deleted `WinSockNetEvents.h` (143 lines)
  - Added `#ifdef FRONTIER_HEADLESS` guards for remaining callers
- **Testing**: 640 lines of integration tests in `tcp_phase2_verbs.yaml`
- **Follow-up**: Issue #362 filed for configurable throughput window (P1)
- **Milestone**: TCP processor now at 100% (23/23 verbs), networking stack production-ready

---

## 2026-01-27 Archive (TCP Networking Phase 1, Threading Foundation, Database Migration Fixes)

Archived content from January 19-24, 2026 covering TCP Phase 1 completion, threading infrastructure, and path resolution fixes.
...
### Earlier Lang/File Verb Implementations
