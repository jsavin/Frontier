...
- Owner: Codex
- Notes: Master checklist of Carbon-era dependencies; update statuses as items move.
...
| Moveable handles | `Ptr`, `Handle`, `NewHandle`, `MemError`, `MaxBlock` | `memory.c`, `portable_handles.h` | @assistant | in-progress | 2025-10-31: portable shim now records `MemError`/`MaxBlock`; next step is to route remaining callers through the shared API. |
...
| Resource manager | `GetString`, `Str255` resource tables | `strings.c`, `langhash.c` | @assistant | in-progress | Libyaml-backed generator (`tools/strings_compiler`) emits tables; next step is to wire runtime lookup helpers and retire legacy Resource Manager paths. |
