...
- Tests: unit coverage for adapter (legacy header → widened in-memory, cached for wide writes) **Done for header path**; table writer/repack BE64 regression and record reference BE64 check added; procedural BE goldens still pass; runtime/CLI suites pass (clock.now still skipped due to frontier-cli exit=1).
