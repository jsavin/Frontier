...
HEADLESS_STUBS = \
    ...
    $(TESTSDIR)/headless_string_verbs.c \
...
Create UserTalk test scripts like:

```
// tests/verb_tests/string.txt
local (result);

// Test string.length
result = string.length("hello");
if (result == 5) {
    dialog.alert("string.length works!");
} else {
    dialog.alert("string.length broken: expected 5, got " + string.length("hello"));
}

// Test string.upper
result = string.upper("hello");
if (result == "HELLO") {
    dialog.alert("string.upper works!");
} else {
    dialog.alert("string.upper broken");
}

// ... test all string verbs ...
```

### 2.3 Test Execution & Reporting

```bash
for processor in file frontier string lang date table dialog ...; do
    echo "=== Testing $processor processor ==="
    frontier-cli tests/verb_tests/$processor.txt 2>&1 | tee results/$processor.txt
done
```
...
```
tests/
├── headless_file_verbs.c              (existing)
├── headless_frontier_verbs.c          (existing)
├── headless_string_verbs.c            (new)
├── headless_lang_verbs.c              (new)
├── headless_date_verbs.c              (new)
├── headless_table_verbs.c             (new)
├── headless_dialog_verbs.c            (new)
├── headless_database_verbs.c          (new)
├── headless_xml_verbs.c               (new)
├── headless_path_verbs.c              (new)
├── ... (49 total)
├── verb_processor_test.sh             (new - test harness)
└── verb_tests/                        (new - test scripts)
    ├── string.txt
    ├── lang.txt
    ├── date.txt
    └── ... (one per processor)
...
frontier-cli/
└── Makefile                           (updated - all processors in HEADLESS_STUBS)

tools/kernelverbs_parser/
└── parse_kernelverbs.py               (updated - all processors in whitelist)

generated/
└── kernel_verbs_init.c                (regenerated - all 51 processors)

results/                               (new - test reports)
├── summary.txt
├── by_status.txt
├── string.txt
├── lang.txt
...
- **Day 1**: Generate all 49 stubs, update build system
- **Day 2**: Verify compilation, fix any issues
- **Day 3**: Create test harness, run initial tests
- **Days 4-5**: Analyze results, triage issues
- **Week 2+**: Iteratively implement and test
