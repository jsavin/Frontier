...
Recent Updates
- **2025-12-23 (Mode Stack Refactor - Active Implementation)**: Significant progress on Phase 1 execution. Completed 5 commits eliminating critical push/pop patterns: (1) Mode stack corruption fix - cleared mode depth before v7 apply to prevent stacked v6 override; (2) External materialization for tables/outlines/scripts/wptext/pictures with correct recursion (tables only, not leaf nodes); (3) Picture externals refactored with `pictverbinmemory()` and `pictverbpack_internal()` taking explicit context; (4) Fixed `langexternalpack_internal()` to set v7 write mode for ALL externals (not just loaded-from-disk); (5) Removed push/pop from `wp_portable_state_dbref()`. Branch: `refactor/explicit-context-no-mode-stack`. Known issues: 22 remaining `dbpushdatabase` calls causing cosmetic mode warnings (harmless - mode lock blocks downgrades). Memory concern raised (Issue #136): materialization loads all externals into memory; optimization deferred until push/pop elimination complete. Progress tracked in `planning/phase3/mode_stack_refactor/MODE_STACK_REFACTOR_PROGRESS.md`.
