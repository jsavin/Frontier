...
5. **Portable header guard cleanup** (Assistant) — 🔄: Follow the staged plan in [`header_cleanup_plan.md`](carbon_migration/header_cleanup_plan.md): (a) extract QuickDraw/UI types into desktop-only code, (b) move TextEncoding/TEC macros to a single portable shim, (c) isolate AppleEvent/Desktop typedefs, then (d) audit include order and rebuild key targets.
