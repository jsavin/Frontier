...
static void eval_cli(const char *script, char *output, size_t output_size) {
	...
	char cmd[16384];
	snprintf(cmd, sizeof(cmd), "cd \"%s\" && FRONTIER_HEADLESS_SKIP_STARTUP=1 %s --system-root \"%s/databases/Frontier-v6-v7.root\" -e \"%s\" 2>&1", root, cli_path, root, escaped_script);
...
int main(int argc, char **argv) {
	...
	printf("[table_operations_integration] Tests run via frontier-cli with Frontier-v6-v7.root\n");
