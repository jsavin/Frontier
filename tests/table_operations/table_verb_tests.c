...
static void eval_cli(const char *script, char *output, size_t output_size) {
    ...
    if (access(db_path, R_OK) != 0) {
        ...
        fprintf(stderr, "[table_verb_tests] Ensure Frontier-v6.root7 exists by running CLI once with Frontier-v6.root\n");
        assert(0);
    }
...
int main(int argc, char **argv) {
    ...
    printf("[table_verb_tests] Tests run via frontier-cli with Frontier-v6.root7\n");
    fflush(stdout);
