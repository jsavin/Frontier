...
static int eval_cli(const char *script, char *output, size_t output_size, int *exit_status) {
    ...
    if (access(db_path, R_OK) != 0) {
        ...
        fprintf(stderr, "[refcon_phase2] Ensure Frontier-v7.root exists by running CLI once with Frontier-v6.root\n");
        return 0;
    }
...
int main(int argc, char **argv) {
    ...
    printf("[refcon_phase2] Tests run via frontier-cli with Frontier-v7.root\n");
