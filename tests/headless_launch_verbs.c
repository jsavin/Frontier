/*
 * headless_launch_verbs.c - Launch processor verbs
 *
 * Originally generated by tools/kernelverbs_parser/generate_processor_stubs.py,
 * now contains @SCRIPT_IMPLEMENTED annotations for UserTalk-implemented verbs.
 *
 * DO NOT regenerate - this file contains production annotations.
 *
 * All launch.* verbs are implemented in pure UserTalk (no C kernel calls).
 * See usertalk_scripts/Frontier.root/system/verbs/builtins/launch/
 */

#include "frontier.h"
#include "standard.h"

#include "memory.h"
#include "strings.h"
#include "lang.h"
#include "langinternal.h"
#include "tablestructure.h"

/* Token enum for all verbs in the launch processor */
enum {
    lauv_applemenu = 0,
    lauv_application = 1,
    lauv_appwithdocument = 2,
    lauv_resource = 3,
    lauv_anything = 4
};

static boolean launch_valueproc(short token, hdltreenode hparam1,
                                     tyvaluerecord *vreturned,
                                     bigstring bserror) {
    (void)hparam1;
    (void)vreturned;
    switch(token) {
        case lauv_applemenu:
            /* Verb #0: launch.applemenu - @SCRIPT_IMPLEMENTED (pure UserTalk) */
            if (bserror) copystring(PSTRING("\017", "not implemented"), bserror);
            return false;
        case lauv_application:
            /* Verb #1: launch.application - @SCRIPT_IMPLEMENTED (pure UserTalk) */
            if (bserror) copystring(PSTRING("\017", "not implemented"), bserror);
            return false;
        case lauv_appwithdocument:
            /* Verb #2: launch.appwithdocument - @SCRIPT_IMPLEMENTED (pure UserTalk) */
            if (bserror) copystring(PSTRING("\017", "not implemented"), bserror);
            return false;
        case lauv_resource:
            /* Verb #3: launch.resource - @SCRIPT_IMPLEMENTED (pure UserTalk) */
            if (bserror) copystring(PSTRING("\017", "not implemented"), bserror);
            return false;
        case lauv_anything:
            /* Verb #4: launch.anything - @SCRIPT_IMPLEMENTED (pure UserTalk) */
            if (bserror) copystring(PSTRING("\017", "not implemented"), bserror);
            return false;
        default:
            return false;
    }
}

boolean launchinitverbs(void) {
    hdlhashtable htable = nil;
    bigstring bsname;

    copystring(PSTRING("\006", "launch"), bsname);

    if (!newfunctionprocessor(bsname, &launch_valueproc, false, &htable))
        return false;

    pushhashtable(htable);

    #define ADD_VERB(name, tok) do { \
        bigstring bs; \
        copystring(name, bs); \
        if (!langaddkeyword(bs, tok)) { \
            pophashtable(); \
            return false; \
        } \
    } while(0)

    ADD_VERB(PSTRING("\011", "applemenu"), lauv_applemenu);
    ADD_VERB(PSTRING("\013", "application"), lauv_application);
    ADD_VERB(PSTRING("\017", "appwithdocument"), lauv_appwithdocument);
    ADD_VERB(PSTRING("\010", "resource"), lauv_resource);
    ADD_VERB(PSTRING("\010", "anything"), lauv_anything);

    #undef ADD_VERB

    pophashtable();
    return true;
}
