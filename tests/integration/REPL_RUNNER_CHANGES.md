...
python3 runner.py test_cases/repl_*.yaml --system-root ../../databases/Frontier.root
```

### Run Mixed Batch + REPL Tests
```bash
python3 runner.py test_cases/*.yaml --system-root ../../databases/Frontier.root
```

### Verbose Output
```bash
python3 runner.py test_cases/repl_commands.yaml --system-root ../../databases/Frontier.root --verbose
```
