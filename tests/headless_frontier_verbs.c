/*
 * headless_frontier_verbs.c - Frontier processor verbs
 *
 * Originally generated by tools/kernelverbs_parser/generate_processor_stubs.py,
 * now hand-maintained. Verbs are marked STUB_CUSTOM in stub_config.py to
 * prevent the generator from overwriting hand-implemented code.
 */
...
static boolean frontier_valueproc(short token, hdltreenode hparam1,
    ...
    switch(token) {
        ...
        case frov_reclaimmemory:
             ...
             * Note: original reclaimmemoryfunc returns a long (bytes freed via
             * hashflushcache). Headless returns boolean true since there is no
             * hash cache to flush. */
            if (!langcheckparamcount(hparam1, 0))
                return false;
            setbooleanvalue(true, vreturned);
