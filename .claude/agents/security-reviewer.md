...
description: |
  ...
  Examples:
  - User has just implemented a new authentication endpoint: "I've just written a new login endpoint that handles user credentials and creates JWT tokens. Can you check if it's secure?" → Perform a comprehensive security analysis of the authentication implementation.
  - User has completed a feature involving file uploads: "Just finished the file upload feature. Here's the code that handles incoming files and stores them." → Analyze this file upload implementation for potential security vulnerabilities like path traversal, file type validation issues, and storage security.
  - User has modified database query logic: "I've updated the user search functionality to support more filters." → Check this database query implementation for SQL injection vulnerabilities and proper input sanitization.
