...
- Before archiving or mass-moving planning documents, read each file’s status block. Only move docs whose `State` is `Completed`, `Done`, or clearly deprecated; leave any Draft/In Progress material in place (or restore it) so active plans stay editable.
