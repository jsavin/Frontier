...
| Paige (HERMES fork) | https://github.com/nmatavka/HERMES-Paige | GPL-2.0 | commit `a2fe9b1` (2024-01-19) | Rich text engine powering WPText serialization/migration | Snapshot checked into `third_party/Paige` (no longer a submodule) with our headless platform file `PGPLATFO/PGUNX.C`. `CMakeLists.txt` is patched to append that file and to define `UNIX_COMPILE`, `C_LIBRARY`, and `NO_OS_INLINE` when building off Windows so headless builds pick the portable layer. Headless runtime links `libpaige.a` built from this tree. |
