...
static boolean navigate_path_with_index(const char *path, typathlookupresult *result,
    ...
    /* Also build a resolved name path (using actual names, not indices) */
    ...
    while (component != NULL && current != nil && depth < COMPLETION_MAX_PATH_DEPTH) {
        ...
        long index = -1;  /* -1 means no index */
        char name_part[COMPLETION_MAX_NAME_LEN];
        ...
        if (bracket != NULL) {
            ...
            strncpy(name_part, component, sizeof(name_part) - 1);
        ...
        if (name_part[0] == '\0' && index > 0) {
            ...
            if (!hashgetnthnode(current, index - 1, &hnode) || hnode == nil) {
                if (error_msg && error_bufsize > 0)
                    snprintf(error_msg, error_bufsize, "index %ld out of range", index);
                return false;
            }

            /* Get the name of this node for the resolved path */
            bigstring bsname;
            gethashkey(hnode, bsname);
            char cname[256];
            size_t nlen = stringlength(bsname);
            if (nlen > sizeof(cname) - 1) nlen = sizeof(cname) - 1;
            memcpy(cname, stringbaseaddress(bsname), nlen);
            cname[nlen] = '\0';

            /* Append to name path */
            if (name_path_len > 0) {
                name_path[name_path_len++] = '.';
            }
            strncpy(name_path + name_path_len, cname, sizeof(name_path) - name_path_len - 1);
            ...
            if (!hashtablelookup(current, bs, &val, &node)) {
                ...
                if (first_component) {
                    ...
                    if (path_result != nil) {
                        ...
                        /* For paths-resolved names, use the actual resolved path */
                        ...
                        if (index > 0) {
                            ...
                            if (!hashgetnthnode(current, index - 1, &hnode) || hnode == nil) {
                                if (error_msg && error_bufsize > 0)
                                    snprintf(error_msg, error_bufsize, "index %ld out of range in '%s'", index, name_part);
                                return false;
                            }
                            ...
                            if (is_last) {
                                hdlhashtable indexed_table = nil;
                                if (langexternalvaltotable(nodeval, &indexed_table, hnode) && indexed_table != nil) {
                                    result->htable = indexed_table;
                                    result->is_table = true;
                                } else {
                                    result->is_table = false;
                                }
                            ...
                            if (!langexternalvaltotable(nodeval, &indexed_table, hnode) || indexed_table == nil) {
                                if (error_msg && error_bufsize > 0)
                                    snprintf(error_msg, error_bufsize, "item at index %ld in '%s' is not a table", index, name_part);
                                return false;
                            }
                            current = indexed_table;
            ...
            strncpy(name_path + name_path_len, name_part, sizeof(name_path) - name_path_len - 1);
            name_path_len = strlen(name_path);
            ...
            if (index > 0) {
                ...
                hdlhashnode hnode = nil;
                if (!hashgetnthnode(subtable, index - 1, &hnode) || hnode == nil) {
                    if (error_msg && error_bufsize > 0)
                        snprintf(error_msg, error_bufsize, "index %ld out of range in '%s'", index, name_part);
                    return false;
                }

                /* Get the actual name of the indexed node */
                bigstring bsname;
                gethashkey(hnode, bsname);
                char cname[256];
                size_t nlen = stringlength(bsname);
                ...
                /* Append the indexed item's name as a child of the current table */
                if (name_path_len > 0) {
                    name_path[name_path_len++] = '.';
                }
                strncpy(name_path + name_path_len, cname, sizeof(name_path) - name_path_len - 1);
    ...
    /* Reached the end without hitting a final component - return current table */
...
boolean repl_resolve_path_ex(const char *path, typathlookupresult *result,
    ...
    /* Check if single-component (no dots, ignoring dots inside brackets) */
 ...
 *   - [n] index syntax like "system.compiler.files[1]" (1-based)
...
boolean repl_jump_path(const char *path) {
    ...
    /* If path has index syntax, use the extended resolver */
    if (path_has_index_syntax(path_buf)) {
        ...
        if (current != nil && current != roottable && g_repl_current_path[0] != '\0') {
            char abs_path[REPL_PATH_MAX_LEN];
            snprintf(abs_path, sizeof(abs_path), "%s.%s", g_repl_current_path, path_buf);
            found = navigate_path_with_index(abs_path, &result, resolved_path, sizeof(resolved_path),
                                              error_msg, sizeof(error_msg));
        }

        if (!found) {
            found = navigate_path_with_index(path_buf, &result, resolved_path, sizeof(resolved_path),
                                              error_msg, sizeof(error_msg));
        }
